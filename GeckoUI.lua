...
GeckoUI.BaseTheme = {
    ...
    -- Particle defaults
    ParticleColor1   = Color3.fromRGB(0, 255, 150),
    ParticleColor2   = Color3.fromRGB(0, 200, 100),
    ParticleMinSize  = 4,
    ParticleMaxSize  = 8,
...
function GeckoUI.CreateWindow(opts)
    ...
    settingsPanel.Size = UDim2.new(0, 260, 0, 430)
    ...
    local function openColorPicker(row)
        ...
        dialog.BackgroundColor3 = Color3.fromRGB(18,28,23)
        dialog.BorderSizePixel = 0
        dialog.Size = UDim2.new(0, 280, 0, 200)
        ...
        local function makeSlider(y,labelText,initial,cb)
            ...
            track.Parent = frame
            track.BackgroundColor3 = Color3.fromRGB(40,60,50)
        ...
        makeSlider(115,"G",g,function(v) g=v applyColor() end)
        makeSlider(150,"B",b,function(v) b=v applyColor() end)
    ...
    local function addColorRow(labelText, themeKey, order)
        ...
        pickBtn.BackgroundColor3 = Color3.fromRGB(25, 40, 30)
    ...
    addColorRow("Particle A",    "ParticleColor1",   7)
    addColorRow("Particle B",    "ParticleColor2",   8)

    local function addNumberRow(labelText, themeKey, order)
        local y = 46 + (order - 1) * 38
        local lbl = Instance.new("TextLabel")
        lbl.Parent = settingsPanel
        lbl.BackgroundTransparency = 1
    ...
    addNumberRow("Particle Min Size", "ParticleMinSize", 9)
    addNumberRow("Particle Max Size", "ParticleMaxSize", 10)
    addNumberRow("Particle Min Delay", "ParticleMinDelay", 11)
    addNumberRow("Particle Max Delay", "ParticleMaxDelay", 12)
    ...
    function window:CreateToggle(props)
        ...
        local function updateVisual()
            if state then
                TweenService:Create(bg, TweenInfo.new(0.12, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {BackgroundColor3 = theme.ToggleOn}):Play()
                TweenService:Create(knob, TweenInfo.new(0.12, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {Position = UDim2.new(1, -18, 0, 2)}):Play()
            else
                TweenService:Create(bg, TweenInfo.new(0.12, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {BackgroundColor3 = theme.ToggleOff}):Play()
                TweenService:Create(knob, TweenInfo.new(0.12, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {Position = UDim2.new(0, 2, 0, 2)}):Play()
            end
        end
